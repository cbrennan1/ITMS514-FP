...
```
# The Chances of fire are greater when less Rain Precepiration
...
```
## qqplot gives us the idea about the distribution of data. In the above graphs it can be seen that Tempreture, Rain, RH and Ws are having data, aproximatly noramlly distributed as most of the points lies on the qqline.
...
```{r}
fr<- alfire %>% subset(Classes == "fire", select= RH)
nofr <- alfire %>% subset(Classes == "not fire", select= RH)
t.test(nofr, mu = 68)
```
...
Region1Training <- alfire[i==1,]
Region1Testing<- alfire[i==2,]          
...
## According to MAE, MSE, R-Square and  Resedual Standard Error Backward stepwise model would be suitable fit over the forward stepwise model.  
...
```{r}
intrain=createDataPartition(y=forestfireregion1$Classes,p=0.7,list=F) # Creating a data partition for training and testing the dataset.
